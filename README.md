...
> daemon 需要本机能运行 `claude`；装了 Antigravity CLI（`agy`）的话它是第二个可选 agent，新建项目时在那一行行尾的字母小标上换（换已有项目的 agent 这件事不存在）。agy 没有 hooks，状态靠屏幕差分；**消息流有**（v1.30 接上，v1.35 重做：工具名、入参与思考都从它的 transcript 里读出来）。项目根放在外置卷（`/Volumes/…`）时要给 `aaa-daemon` 开完全磁盘访问权限并用固定签名，见 `daemon/README.md`。

## 用法

- **Mac App**：左栏是项目列表（一项目一行：标题 + 更新时间，**整行淡底色说状态**——**淡蓝 = 在跑**、**淡黄 = 跑完了 / 在等你回话而这台机器还没看过**、**无底色 = 已读**，不写状态字；当前打开的那一行标题用强调色、整行套一圈强调色边框；按更新时间从新到旧排），**列表末尾一行就是新建项目**——长得和项目行一样，输文件夹名回车即建，行尾的字母小标选新项目开谁；左下角是 plan 配额。⌘N 光标进那一行、⌃Tab 切会话、⌘E 消息流与终端互切、⌘I 右侧详情栏（会话用量与提示缓存命中、对话进度清单、**子代理**、**后台任务**、**已上传**、**产物**——发布过的链接加这个项目里的 Markdown，点一下就在会话区读、**已使用技能**）、侧栏底部「看板」（所有会话的进度，瀑布流全展开：进度条、全部清单项；分「在 AAA 里」/「不在 AAA 里」两节，后者默认折起）、⌘W 关闭。消息流里 Claude 的每段回答都露出来，中间的思考与工具步骤折叠成一行，📎 上传文件后以 `@路径` 引用；Claude 要授权（Bash 命令、批准计划）时消息流末尾出现「允许 / 拒绝」卡片，不用切到终端。
- **Android**：Claude 跑完一轮收到「完成」通知，点开直达会话；提问以原生表单作答；系统分享或 📎 把文件传进项目；首页是项目列表（一行一个，标题 + 时间，**整行淡底色说状态**：**淡蓝 = 在跑**、**淡黄 = 跑完了 / 在等你回话**（点进去就没了）、**无底色 = 已读**；当前那一行标题用强调色、整行套一圈强调色边框；按更新时间从新到旧排），顶栏就一行——左边 plan 配额（5h、7d 与 Fable 周窗口，点开看重置时间），右边 ▦ 看板与 ⚙ 设置，**项目列表下面就是终端列表**（一行一个，行尾 × 关掉，底部「＋ 新增终端」——终端和会话是平级的两种东西，不再藏在顶栏一个按钮后面）。会话页顶栏只有一行：标题 · 模型 · 上下文占比；右上角**不再是 ⋮ 而是详情**（一个 42dp 见方的按钮，☰ 同样大）——进度清单、用量、子代理、后台任务、已上传、**产物**（发布过的链接加这个项目里的 Markdown，点一行就地读）、已使用技能，还有重命名 / 重启 agent 这些低频操作都收在里面（会话的生杀归项目列表长按，不在对话里管对话）。会话页左上角 ☰ 拉出的就是完整项目面板（同一根顶栏、项目列表与长按操作、**列表末尾那一行就是新建项目**、终端列表），app 起来直接回上次的会话，没有单独的首页。▦ 是看板：所有会话的进度——瀑布流全展开的卡片（进度条 + 全部清单项），点清单项直接勾 / 取消勾；分「在 AAA 里」/「不在 AAA 里」两节，后者默认折起。消息流里 Claude 还在跑时点「发送」，那句话就挂在末尾标「待发送」，跑完自动发出——**排队是 Claude Code 自己的行为**（和你在终端里先敲好等它一样），AAA 只是把它的队列画出来，两端都画；输入框的字切出去再回来还在。终端视图的快捷键条在画面上方，回车在最前；终端字号固定。
