...
- **Mac App**：左栏是项目列表（一项目一行，行首写 待回复 / 运行 / 后台 / 激活 / 暂停；置顶的在最前，其余按这个顺序排，同状态里最近更新的在前），顶部输入框输文件夹名回车即新建项目，左下角是 plan 配额。⌘N 新建、⌃Tab 切会话、⌘E 消息流与终端互切、⌘I 右侧详情栏（会话用量与提示缓存命中、对话进度清单、发布过的产物链接、静音）、侧栏底部「看板」（所有会话的进度，瀑布流全展开：状态、进度条、全部清单项）；行尾悬停「归档」把项目从列表和看板藏起来，不删任何东西、不用确认，脚注「归档 N」翻出来、⌘W 关闭。消息流里 Claude 的每段回答都露出来，中间的思考与工具步骤折叠成一行，📎 上传文件后以 `@路径` 引用；Claude 要授权（Bash 命令、批准计划）时消息流末尾出现「允许 / 拒绝」卡片，不用切到终端。
- **Android**：Claude 跑完一轮收到「完成」通知，点开直达会话；提问以原生表单作答；系统分享或 📎 把文件传进项目；首页是项目列表（一行一个，状态字 + 标题；按 待回复 > 运行 > 后台 > 激活 > 暂停 排，同状态里最近更新的在前），顶部是 plan 配额（5h、7d 与 Fable 周窗口），会话菜单里有「进度」（整个对话的 todo list：做了什么、还没做什么，每轮结束后更新）和「产物」。会话页左上角 ☰ 拉出的就是完整项目面板（首页整块：连接状态、终端、看板、设置、用量、新建、项目列表与长按操作），app 起来直接回上次的会话，没有单独的首页。▦ 是看板：所有会话的进度——五态计数、瀑布流全展开的卡片（进度条 + 全部清单项）；长按项目可「归档」（不删、不确认）；暂停超过 14 天且清单全勾完的项目每小时自动归档（`auto_archive_days`）；看板上点清单项直接勾 / 取消勾。消息流里 Claude 还在跑时点「发送」，那句话排成「待发送」挂在末尾，跑完自动发出（✕ 撤回）；输入框的字切出去再回来还在。终端视图的快捷键条在画面上方，回车在最前；终端字号固定，不再提供字号设置。
