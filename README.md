...
- **Mac App**：左栏是项目列表（一项目一行，行首写 执行中 / 待回复 / 已激活 / 未激活，最近更新的在前），顶部输入框输文件夹名回车即新建项目，左下角是 plan 配额。⌘N 新建、⌃Tab 切会话、⌘E 消息流与终端互切、⌘I 右侧详情栏（会话用量与提示缓存命中、对话进度清单、发布过的产物链接、收件箱、静音）、侧栏底部「看板」（左边所有会话里没勾的待办、右边按天的「这一天做了什么」）、⌘W 关闭。消息流里 Claude 的每段回答都露出来，中间的思考与工具步骤折叠成一行，📎 上传文件后以 `@路径` 引用。
